...
export function RunCaseDetailPage() {
  ...
  const started = caseExec.started_at
    ? new Date(caseExec.started_at).getTime()
    : Date.now();
  const finished = caseExec.finished_at
                ...
                {Array.isArray(observation.retrieval) ? (
                  observation.retrieval
                    ...
                    .map((item: unknown, idx: number) => (
                      <div
                        key={idx}
                        className="border-border rounded border bg-surface p-3"
                      >
                  ...
                  <div className="text-sm text-text-secondary">
                    {JSON.stringify(observation.retrieval)}
                  </div>
                )}
              </div>
            </Surface>
