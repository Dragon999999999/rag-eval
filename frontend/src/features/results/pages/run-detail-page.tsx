...
export function RunDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "cases" | "metrics">("overview");
      ...
      <Page.Content>
        <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-4 gap-4">
                  <div>
                    ...
                    <div className="text-lg font-semibold text-success">{progress.complete_cases}</div>
                  </div>
                  <div>
                    <div className="text-xs text-text-tertiary">Failed</div>
                    <div className="text-lg font-semibold text-error">{progress.failed_cases}</div>
                  </div>
                  <div>
                    <div className="text-xs text-text-tertiary">Running</div>
                    <div className="text-lg font-semibold text-info">{progress.running_cases}</div>
                  </div>
                  <div>
                    <div className="text-xs text-text-tertiary">Total</div>
                    <div className="text-lg font-semibold text-text-primary">{progress.total_cases}</div>
                  </div>
                </div>
              </div>
            </Surface>
          ...
          {isCompleted && aggregates && aggregates.length > 0 && (
            <Surface className="p-6">
              <h3 className="mb-4 text-sm font-medium text-text-primary">Key Metrics</h3>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                {aggregates.map((agg: AggregateResultSummary) => (
                  <div key={agg.metric_id} className="space-y-1">
                    <div className="text-xs text-text-tertiary">{agg.metric_id.split(".").pop()}</div>
              ...
              <Surface className="p-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <div className="font-medium text-text-primary">{run.target_id ?? "—"}</div>
                  </div>
                  <div>
                    <div className="text-xs text-text-tertiary">Test Definition</div>
                    <div className="font-medium text-text-primary">
                      {run.test_definition_id ? (
                        <Link to={`/tests/${run.test_definition_id}`} className="text-accent hover:underline">
                          {run.test_definition_id}
                        </Link>
                      ) : (
                        "—"
                  ...
                  <div>
                    ...
                    <div className="font-mono text-xs text-text-secondary">{run.config_hash}</div>
                  </div>
                  <div>
                    <div className="text-xs text-text-tertiary">RAG-Eval Version</div>
                    <div className="font-medium text-text-primary">{run.rag_eval_version ?? "—"}</div>
                  </div>
                </div>
              </Surface>
              ...
              <Surface className="p-4">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  <div>
                    ...
                    <div className="text-sm text-text-secondary">{formatRelativeTime(run.created_at)}</div>
                  </div>
                  <div>
                    <div className="text-xs text-text-tertiary">Started</div>
                    <div className="text-sm text-text-secondary">{formatRelativeTime(run.started_at)}</div>
                  </div>
                  <div>
                    <div className="text-xs text-text-tertiary">Finished</div>
                    <div className="text-sm text-text-secondary">{formatRelativeTime(run.finished_at)}</div>
                  </div>
                </div>
              </Surface>
            </div>
          ...
          {activeTab === "metrics" && (
            <Surface className="p-4">
              <h3 className="mb-4 text-sm font-medium text-text-primary">Metric Results</h3>
              {aggregates && aggregates.length > 0 ? (
                <div className="space-y-4">
                  {aggregates.map((agg: AggregateResultSummary) => (
                    <div key={agg.metric_id} className="flex items-center justify-between border-b border-border pb-3">
                      <div>
                        <div className="font-medium text-text-primary">{agg.metric_id}</div>
                        <div className="text-xs text-text-tertiary">v{agg.metric_version} · {agg.aggregation}</div>
                      </div>
                      <div className="text-lg font-semibold text-text-primary">
                        {formatMetricValue(agg.value_summary, agg.metric_id)}
                      </div>
