...
def test_execution_engine_persists_case_results() -> None:
    """Verify the execution engine persists case results durably."""
    asyncio.run(_execute_test_benchmark())


async def _execute_test_benchmark() -> None:
    ...
    async with session_factory() as session:
        ...
        await repository.create_run(
            run, {"version": "1", "run": {"name": "test-execution"}}
        )
        
        # Register a test benchmark case
        from rag_eval.models import BenchmarkCase
        case = BenchmarkCase(
            case_id=f"test-case-{suffix}",
            query="What is the capital of France?",
        )
        await repository.persist_benchmark_case("test-benchmark", case)
        
        await session.flush()
        
        # Create adapter and artifact service
        ...
        # Load config for execution parameters
        config = load_experiment_config(Path("examples/basic.yaml"))
        
        # Create executor
        executor = BenchmarkExecutor(
            config, adapter, artifact_service, repository, session
        )
        ...
        async with session.begin():
            result = await executor.execute(run.run_id, manifest)
        
        # Verify results
        assert result.total_cases >= 0
        
        # Check case execution was persisted
        case_executions = await repository.list_case_executions(run.run_id)
        assert len(case_executions) > 0
        
        # Check attempts were persisted
        for case_exec in case_executions:
            attempts = await repository.list_attempts(case_exec.case_execution_id)
            # Should have at least one attempt per case
